...
public class Final_project3 {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < fibs.length - 1; i++) {
      long startTime = ThreadMXBean.setThreadCpuTimeEnabled(true);
      int gcd = gcd(fibs[i], fibs[i + 1]);
      long endTime = ThreadMXBean.setThreadCpuTimeEnabled(true);
    ...
    for (int i = 0; i < fibs.length - 1; i++) {
      ...
      long endTime = ThreadMXBean.getCurrentThreadUserTime();
      long executionTime = endTime - startTime;
      System.out.print("     " + executionTime);
    }
  }
